...
use gpui::{
    AtlasTextureId, BackdropBlur, Background, Bounds, DevicePixels, DrawOrder,
    ExternalTextureSprite, GpuSpecs, PaintExternalTexture, Path, Point, PrimitiveBatch,
...
/// released after a run of blur-free frames.
#[derive(Clone)]
...
impl WgpuRenderer {
    ...
    fn record_frame(
...

        {
            ...
            while self.backdrop_blur_supported {
                let Some((blur_index, blur)) = pending_blurs.next() else {
                    break;
                };
                if blur_index >= BACKDROP_MAX_BLURS {
                    continue;
...
fn batch_first_order(scene: &Scene, batch: &PrimitiveBatch) -> DrawOrder {
    match batch {
        ...
        PrimitiveBatch::ExternalTextures(range) => scene.external_textures[range.start].order,
    }
}
